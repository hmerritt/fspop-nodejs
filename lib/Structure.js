...
const { isArray, isString, isPlainObject, has } = require("lodash");
...
class Structure {
    ...
    /**
     * Count all nodes in a structure
     *
     * @param {Object}   structure       Structure
     */
    count(structure) {
        let count = 0;
        this.crawl(structure, "", () => count++);
        return count;
    }

    /**
     * Verify structure config integrity
     *
     * @param {Object}   structure       Structure
     */
    isValid(structure) {
        // All structure files must have/be;
        // -> an object
        // -> property: name
        // -> property: structure
        if (
